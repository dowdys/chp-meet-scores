...
2. Use \`web_search\` as a last resort to find the meet results page
3. Ask the user for help — they may have the URL
...
- If any requested levels are missing, explicitly call that out`,
  ...
  database: {
    ...
    tools: [
      ...
      'list_meets', 'perplexity_gym_lookup', 'rename_gym', 'fix_names',
    ],
...
3. Fix any issues found:
   - **Gym names**: Use \`rename_gym\` to merge/standardize gym name variants
   - **Athlete names**: Use \`fix_names\` to correct suspicious name issues (e.g., event code suffixes like "Anna NicklowBB" → "Anna Nicklow"). Review each suggestion carefully — names like "Cobb" should NOT be changed.
...
\`query_db\` automatically queries the staging database during processing.`,
  ...
  output_finalize: {
    ...
    tools: [
      ...
      'get_meet_summary', 'set_output_name', 'rename_gym', 'fix_names',
    ],
...
### Fixing Issues During Output
- **Suspicious names**: If \`regenerate_output\` detects names with event code suffixes (e.g., "Anna NicklowBB"), it will block and list suggested corrections. Use \`fix_names\` with the meet name and your corrected list, then call \`regenerate_output\` again. Review each suggestion carefully — names like "Emily Cobb" should NOT be changed.
- **Gym names**: Use \`rename_gym\` to fix gym name variants (e.g., "Aspiregymnasticsclub" → "Aspire Gymnastics").
