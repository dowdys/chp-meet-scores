...
Full InDesign document (ZIP of XML files) that opens in InDesign CS6+ as a complete, editable document. IDML includes:
...
IDML is the preferred and only InDesign format. It is a complete document that needs no manual work.
...
Available values: `shirt`, `idml`, `order_forms`, `gym_highlights`, `summary`, `all`.

**IMPORTANT**: Always pass `division_order` on the first `regenerate_output` call for any shirt generation. The agent must provide the division ordering.

**Auto-regenerate**: `--regenerate shirt` automatically also regenerates ALL shirt-dependent outputs: `meet_summary.txt`, `back_of_shirt.idml`, `order_forms.pdf`, and `gym_highlights.pdf`. This ensures they all use the updated layout (page groups, font sizes, spacing). You don't need to list them explicitly.
...
## PDF Import (import_pdf_backs tool)

For importing designer-edited PDF backs from InDesign, use the `import_pdf_backs` tool in the `import_backs` phase. Use `set_phase("import_backs")` to enter this phase. The tool handles combining pages, scaling, and regenerating order forms and gym highlights automatically.

**IMPORTANT**: `--import-idml` is **deprecated** and will error. Use `import_pdf_backs` instead.
...
```bash
cp /home/goduk/chp-meet-scores/data/[meet_slug]/*.{pdf,idml,txt} "/mnt/c/Users/goduk/Downloads/"
```
