...
4. Regenerates `gym_highlights.pdf` (overlay on shirt PDF with yellow highlights)
5. Regenerates `order_forms.pdf` (back pages copy from shirt PDF with red stars)
6. Generates `meet_summary.txt`
7. Copies the IDML to the output folder

**Output files from IDML import**: `back_of_shirt.pdf`, `back_of_shirt.idml`, `gym_highlights.pdf`, `order_forms.pdf`, `meet_summary.txt`. No other files should be generated.

**IMPORTANT**: `--import-idml` is self-contained. Do NOT run `--regenerate` after it — that would overwrite the IDML-imported outputs with code-rendered versions, losing all designer edits.
...
**When to use**: After the user has edited the IDML in InDesign and wants to finalize the shirt back. The imported PDF becomes the definitive version. All designer edits (fonts, colors, spacing, layout changes) automatically propagate to gym_highlights.pdf and order_forms.pdf back pages because those files overlay on the shirt PDF rather than re-rendering independently.
